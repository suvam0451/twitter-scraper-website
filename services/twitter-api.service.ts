...
export const searchUsername = async (query: string) => {
    const res = await MakeClient().get<UserMetaData>(`/users/by/username/${query}`)
    return res.data.data
}

export const fetchNextBatchTweets = async (id: string, refresh_token?: string) => {
    let tweetSet = await MakeClient().get<UserTweetsResponse>(
        `/users/${id}/tweets?max_results=100&tweet.fields=possibly_sensitive${refresh_token == '' ? '' : `&pagination_token=${refresh_token}`
        }`
    );
...
export const resolveImages = async (arr: string[]) => {
    let retval: string[] = []
    const tweet_ids_query = arr.reduce((prev, curr, idx, entry) => prev + ',' + curr);
    let res = await MakeClient().get(`/tweets?ids=${tweet_ids_query}&media.fields=url&expansions=attachments.media_keys`)
    .catch((err) => {
        console.log(err);
    })
    if (res) {
...
export const fetchImagesForUsername = async (query: string) => {
    let _refreshToken = ''
    let tweet_count = 0
    let sfwLinks: string[] = [], nsfwLinks: string[] = []

    do {
        let { data, refreshToken } = await fetchNextBatchTweets(query, _refreshToken)
